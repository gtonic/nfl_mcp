...
`project_player` / `project_players` (transparent weekly points) · `get_opportunity_projections` (trailing-volume baseline) · `get_start_sit_recommendation` · `get_roster_recommendations` · `compare_players_for_slot` · `analyze_full_lineup` · `get_win_probability_lineup` (lineup that maximises P(beating *this* opponent))
