...
79 MCP tools over HTTP, grouped by what they do. Every tool ships its own parameter
...
`get_weekly_briefing` (**start here** — roster, opponent, weather, usage and the lineup changes worth making, in one call) · `project_player` / `project_players` (transparent weekly points) · `get_opportunity_projections` (trailing-volume baseline) · `get_start_sit_recommendation` · `get_roster_recommendations` · `compare_players_for_slot` · `analyze_full_lineup` · `get_win_probability_lineup` (lineup that maximises P(beating *this* opponent))
...
**💸 Waivers & FAAB**
`get_waiver_wire_dashboard` · `get_waiver_log` · `check_re_entry_status` · `recommend_faab_bid` · `get_handcuff_map` (your RBs → their backups, and who's free)
...
`get_injury_report` · `get_high_confidence_injuries` (multi-source) · `get_injury_trends` (**what changed** since you last looked, not who's hurt) · `get_gameday_inactives`
